...
export default class EventHandlerImp {
  ...
  private _mouseEnterHandler (enterEvent: MouseEvent): void {
    this._unsubscribeMousemove?.()
    // 不再在 mouseenter 时注册/移除滚轮事件，因为已经在 _init 中注册了
    // this._unsubscribeMouseWheel?.()
    this._unsubscribeContextMenu?.()
  ...
  private _mouseWheelHandler (wheelEvent: WheelEvent): void {
    // 先检查鼠标位置是否在当前容器内
    // 这是最重要的检查，确保只有鼠标所在的那个图表实例处理滚轮事件
    const rect = this._target.getBoundingClientRect()
    const mouseX = wheelEvent.clientX
    const mouseY = wheelEvent.clientY
    
    // 如果鼠标不在当前容器内，立即返回，不处理事件
    if (mouseX < rect.left || mouseX > rect.right || mouseY < rect.top || mouseY > rect.bottom) {
      return
    }
    
    // 检查事件目标是否在当前图表容器内
    const target = wheelEvent.target as Node
    const currentTarget = wheelEvent.currentTarget as Node
    const targetInContainer = target && this._target.contains(target)
    const currentTargetIsThis = currentTarget === this._target
    
    // 如果鼠标在当前容器内，但事件目标不在当前容器内，且 currentTarget 也不是当前容器，不处理
    if (!targetInContainer && !currentTargetIsThis) {
      return
    }
    
    // 如果鼠标在当前容器内，且事件目标也在当前容器内，就应该处理这个事件
    // 在处理事件之前，立即调用 stopImmediatePropagation 阻止其他监听器处理同一个事件
    // 这样可以确保只有鼠标所在的图表实例处理事件
  ...
  private _init (): void {
    ...
    // 立即注册滚轮事件监听器，不等待 mouseenter
    // 使用冒泡阶段，但通过鼠标位置检查确保只有正确的实例处理事件
    // 这样可以确保即使鼠标没有进入容器，滚轮事件也能被处理
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    const boundMouseWheel = this._mouseWheelHandler.bind(this)
    this._unsubscribeMouseWheel = () => {
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
      this._target.removeEventListener('wheel', boundMouseWheel, { passive: false } as any)
    }
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    // 使用冒泡阶段，通过鼠标位置检查确保只有正确的实例处理事件
  ...
  private _mouseLeaveHandler (event: MouseEvent): void {
    this._unsubscribeMousemove?.()
    // 不再在 mouseleave 时移除滚轮事件，因为滚轮事件应该始终可用
    // this._unsubscribeMouseWheel?.()
    this._unsubscribeContextMenu?.()
