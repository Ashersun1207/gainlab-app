...
export default class CrosshairHorizontalLabelView<C extends Axis = YAxis> extends View<C> {
  ...
  protected getText (crosshair: Crosshair, chartStore: ChartStore, axis: Axis): string {
    ...
    let value: number
    if (typeof crosshair.y === 'number') {
      value = yAxis.convertFromPixel(crosshair.y)
    } else {
      value = axis.convertFromPixel(0)
    }
    let precision = 0
    ...
    if (yAxis.isInCandle()) {
      ...
      // 处理脚本的精度
      const scripts = chartStore.getScriptsByPaneId(crosshair.paneId!)
