...
export interface Store {
  ...
  addScript: (script: any, paneId: string, isStack: boolean) => string | null
  getScriptsByPaneId: (paneId: string) => any[]
  getScriptsByFilter: (filter: any) => any[]
  removeScript: (filter?: any) => boolean
...
export default class StoreImp implements Store {
  ...
  /**
   * Whether subscribeBar has been registered
   */
  private _hasSubscribedBars = false

  /**
   * Realtime update throttle state
   */
  // 还原为直接更新的模式（不使用合并/节流），避免副作�?
  // 保留占位，便于将来需要时再启�?
  // private _realtimeTimer: ReturnType<typeof setTimeout> | null = null
  // private _lastRealtimeData: Nullable<KLineData> = null
  // private _realtimeThrottleMs = 32
  // private _realtimePausedUntil = 0
  ...
  * Whether there are forward and backward more flag
  ...
  private readonly _scripts = new Map<string, any[]>()
  // Track panes whose y-axis has been fully initialized (first data ready)
  private readonly _paneYAxisInited = new Set<string>()
  ...
  setStyles (value: string | DeepPartial<Styles>): void {
    ...
    const candleType = this._styles.candle.type
    if (candleType === 'footprint' || candleType === 'footprint_period') {
      BarSpaceLimitConstants.MAX = 120
      BarSpaceLimitConstants.MIN = 42
      if (this._barSpace < BarSpaceLimitConstants.MIN) {
        this._barSpace = BarSpaceLimitConstants.MIN
      }
    } else {
      BarSpaceLimitConstants.MIN = 1
      BarSpaceLimitConstants.MAX = 50
      if (this._barSpace > BarSpaceLimitConstants.MAX) {
        this._barSpace = BarSpaceLimitConstants.MAX
      }
    }
  ...
  private _addData (
    ...
    if (isArray<KLineData>(data)) {
      ...
      if (timestamp > lastDataTimestamp) {
        ...
        // 处理历史数据插入（时间戳小于最后一条数据的情况�?
        // 找到正确的插入位�?
        let insertIndex = dataCount - 1
        for (let i = dataCount - 2; i >= 0; i--) {
          const currentTimestamp = formatValue(this._dataList[i], 'timestamp', 0) as number
        ...
        // 在正确位置插入数�?
        this._dataList.splice(insertIndex, 0, data)
        dataLengthChange = 1
        success = true
        adjustFlag = true
      }
    ...
    if (success) {
      if (adjustFlag) {
        ...
        // 检查脚本状态变化
        this._chart.checkScriptStateChanges().catch(error => {
  ...
  setDataList (data: KLineData[]): void {
    // 保持现有�?forward/backward 开关，不做修改
    const prevMore = { backward: this._dataLoadMore.backward, forward: this._dataLoadMore.forward }
    this._addData(data, 'init', prevMore)
    
    // 如果�?dataLoader，且未注册过 subscribeBar，则注册一�?
    if (!this._hasSubscribedBars && isValid(this._dataLoader) && isValid(this._symbol) && isValid(this._period)) {
      ...
      this._hasSubscribedBars = true
    }
  }

  /**
   * 批量 upsert（逐条�?update 规则：时间戳<插入�?覆盖�?末尾追加），仅一次布局
   */
  upsertDataList (dataList: KLineData[]): void {
    if (!isArray<KLineData>(dataList) || dataList.length === 0) return
    // 按时间戳升序，减少插入定位成�?
    const bars = [...dataList].sort((a, b) => a.timestamp - b.timestamp)

    // 关闭多余布局：通过一次性触发最终更�?
    let changed = false
    for (const data of bars) {
      const dataCount = this._dataList.length
      const timestamp = data.timestamp
      const lastTs = formatValue(this._dataList[dataCount - 1], 'timestamp', 0) as number
      if (dataCount === 0 || timestamp > lastTs) {
        this._dataList.push(data)
        let lastBarRightSideDiffBarCount = this.getLastBarRightSideDiffBarCount()
        if (lastBarRightSideDiffBarCount < 0) {
          this.setLastBarRightSideDiffBarCount(--lastBarRightSideDiffBarCount)
      ...
      let insertIndex = dataCount - 1
      for (let i = dataCount - 2; i >= 0; i--) {
        const curTs = formatValue(this._dataList[i], 'timestamp', 0) as number
        if (timestamp > curTs) {
          insertIndex = i + 1
          break
        } else if (timestamp === curTs) {
          this._dataList[i] = data
          changed = true
          insertIndex = -1
          break
    ...
    if (changed) {
      ...
      this._chart.checkScriptStateChanges().catch(() => {})
      this._chart.layout({
        measureWidth: true,
        update: true,
        buildYAxisTick: true,
  ...
  /**
   * 批量追加末端数据（比逐条 subscribe 回调更高效）
   */
  appendDataList (data: KLineData[]): void {
    if (!isArray<KLineData>(data) || data.length === 0) return
    const prevMore = { backward: this._dataLoadMore.backward, forward: this._dataLoadMore.forward }
    this._addData(data, 'backward', prevMore)
  }
  ...
  setBarSpace (barSpace: number, adjustBeforeFunc?: () => void): void {
    // 根据当前图表的类型动态获取限制，而不是使用全局常量
    // 这样可以避免多个图表实例之间的冲突
    const candleType = this._styles.candle.type
    const minBarSpace = (candleType === 'footprint' || candleType === 'footprint_period') ? 42 : 1
    const maxBarSpace = (candleType === 'footprint' || candleType === 'footprint_period') ? 120 : 50
    
    if (barSpace < minBarSpace || barSpace > maxBarSpace || this._barSpace === barSpace) {
      return
    }
  ...
  dataIndexToCoordinate (dataIndex: number): number {
    const dataCount = this._dataList.length
    // 对任�?dataIndex（包括超出范围的负数/大于最后一根）都按统一公式线性换算，避免在最后一根右侧坍缩到同一坐标
  ...
  setCrosshair (
    ...
    if (isNumber(cr.x)) {
      ...
      if (realDataIndex < 0 || realDataIndex > lastIndex) {
        // 鼠标在数据范围之外时，直接使用鼠标的实际像素坐标，确保竖线能继续跟随
        realX = cr.x
      } else {
        realX = this.dataIndexToCoordinate(realDataIndex)
      }
    } else {
      realX = this.dataIndexToCoordinate(realDataIndex)
    }
    ...
    this._taskScheduler.addTask({
      ...
      handler: () => {
        ...
        indicator.calcImp(this._dataList).then(result => {
          if (result) {
            // 首次数据就绪（或init）时，强制该pane重建Y轴（不使用缓存），修复刷新后副图Y轴不更新问题
            const paneId = indicator.paneId
            const forceYAxis = dataLoadType === 'init' || !this._paneYAxisInited.has(paneId)
            if (forceYAxis) {
              // 先让�?pane 自身执行一次主流程更新，确保内部数据域、范围已同步
              this._chart.updatePane(UpdateLevel.Main, paneId)
            }
            this._chart.layout({
              measureWidth: true,
              update: true,
              buildYAxisTick: true,
              cacheYAxisWidth: forceYAxis ? false : true
            })
            if (forceYAxis) {
              this._paneYAxisInited.add(paneId)
              // 进一步触发该pane内部y轴的完整初始化（一次性）
              try { this._chart.setPaneOptions({ id: paneId }) } catch (_) {}
            }
            indicator.onDataStateChange?.({
              state: 'ready',
              type: dataLoadType,
  ...
  addScript (script: any, paneId: string, isStack: boolean): string | null {
    const paneScripts = this._scripts.get(paneId) ?? []
    if (isStack) {
      paneScripts.push(script)
    } else {
      paneScripts.unshift(script)
    }
    this._scripts.set(paneId, paneScripts)
    return script.key
  }


  private _sortScripts (paneId?: string): void {
    // 占位符：脚本排序逻辑
  }

  getScriptsByPaneId (paneId: string): any[] {
    return this._scripts.get(paneId) ?? []
  }

  getScriptsByFilter (filter: any): any[] {
    const { paneId, name, key } = filter
    
    const match: ((script: any) => boolean) = script => {
      if (isValid(key)) {
        return script.key === key;
      }
      return !isValid(name) || script.name === name
    }
    let scripts: any[] = []
    if (isValid(paneId)) {
      const paneScripts = this.getScriptsByPaneId(paneId);
      scripts = scripts.concat(paneScripts.filter(match))
    } else {
      this._scripts.forEach(paneScripts => {
        scripts = scripts.concat(paneScripts.filter(match))
      })
    }
    return scripts
  ...
  removeScript (filter?: any): boolean {
    let removed = false
    const filterScripts = this.getScriptsByFilter(filter ?? {})
    
    filterScripts.forEach(script => {
      const paneScripts = this.getScriptsByPaneId(script.paneId)
      
      const index = paneScripts.findIndex(s => s.key === script.key)
      
      if (index > -1) {
        paneScripts.splice(index, 1)
        removed = true
      }
      if (paneScripts.length === 0) {
        this._scripts.delete(script.paneId)
      }
    })
    return removed
  }
  ...
  hasScripts (paneId: string): boolean {
    return this._scripts.has(paneId)
  }

  overrideScript (override: any): boolean {
    let updateFlag = false
    const filterScripts = this.getScriptsByFilter(override)
    
    filterScripts.forEach(script => {
      // 检查脚本是否需要更新
      if (script.shouldUpdateImp && script.shouldUpdateImp()) {
        updateFlag = true
      }
    })
  ...
  getOverlaysByFilter (filter: OverlayFilter): OverlayImp[] {
    const { id, groupId, group, paneId, name } = filter
    const match: ((overlay: OverlayImp) => boolean) = overlay => {
      if (isValid(id)) {
        ...
        if (isValid(group)) {
          return overlay.group === group && (!isValid(name) || overlay.name === name)
        }
      }
  ...
  removeOverlays (ids: string[]): boolean {
    ...
    const updatePaneIds: string[] = []
    ids.forEach(id => {
      const filterOverlays = this.getOverlaysByFilter({ id })
      filterOverlays.forEach(overlay => {
        const paneId = overlay.paneId
        const paneOverlays = this.getOverlaysByPaneId(overlay.paneId)
        overlay.onRemoved?.({ overlay, chart: this._chart })
  ...
  setPriceVolumePrecision(options: PrecisionOptions): void {
    if (this._symbol) {
      if (typeof options.pricePrecision === 'number') {
        this._symbol.pricePrecision = options.pricePrecision
      }
      if (typeof options.volumePrecision === 'number') {
        this._symbol.volumePrecision = options.volumePrecision
      }
    }
    this._synchronizeIndicatorSeriesPrecision()
    this._chart.layout({ update: true })
