    ...
    if (coordinates.length > 1) {
      ...
      if (kb) {
        ...
        if (coordinates[1].y > coordinates[0].y) {
          ...
          offsetAngle = Math.PI / 3 * 3
        }
      }
      const rotateCoordinate1 = getRotateCoordinate({ x: coordinates[1].x - 18, y: coordinates[1].y + 6 }, coordinates[1], offsetAngle)
      const rotateCoordinate2 = getRotateCoordinate({ x: coordinates[1].x - 18, y: coordinates[1].y - 6 }, coordinates[1], offsetAngle)
      return [
        {
          type: 'line',
          attrs: { coordinates }
