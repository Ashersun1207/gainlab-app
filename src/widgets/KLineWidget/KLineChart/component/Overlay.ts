...
export interface Overlay<E = unknown> extends OverlayEventCollection<E> {
  ...
  /**
   * Custom group name for categorization (e.g., 'user_manual', 'order_entry', 'news_important')
   */
  group: string
...
export type OverlayFilter<E = unknown> = Partial<Pick<Overlay<E>, 'id' | 'groupId' | 'group' | 'name' | 'paneId'>>
...
export default class OverlayImp<E = unknown> implements Overlay<E> {
  ...
  setHasMoved(val: boolean) { this._hasMoved = val }
  getHasMoved() { return this._hasMoved }
}
