...
export default class OverlayView<C extends Axis = YAxis> extends View<C> {
  ...
  private _isMouseInButtonArea(event: MouseTouchEvent): boolean {
    // 获取 widget 的 bounding box
    const widget = this.getWidget()
    const bounding = widget.getBounding()
    
    // 计算转换后的坐标
    const convertedX = event.x - (bounding?.left ?? 0)
    const convertedY = event.y - (bounding?.top ?? 0)
    
    // 获取 CandleTooltipView 的按钮区域
    const pane = widget.getPane()
    const mainWidget = pane.getMainWidget()
    const children = (mainWidget as any)._children || []
    
    for (const child of children) {
      if (child.constructor.name === 'CandleTooltipView') {
        const buttonAreas = child.getButtonAreas()
        
        for (const area of buttonAreas) {
          if (
            convertedX >= area.left && 
            convertedX <= area.right &&
            convertedY >= area.top && 
            convertedY <= area.bottom
          ) {
            return true
          }
        }
  ...
  private _initEvent (): void {
    ...
    this.registerEvent('mouseMoveEvent', event => {
      ...
      // 检查是否点击在脚本按钮上
      const widget = this.getWidget()
      if (widget.getName() === WidgetNameConstants.SCRIPT) {
        const scriptWidget = widget as any
        if (scriptWidget._isMouseInScriptButtons && scriptWidget._isMouseInScriptButtons(event)) {
          // 如果点击在脚本按钮上，不处理overlay事件
          return false
        }
      }
      ...
      if (overlay !== null) {
        if (checkOverlayFigureEvent('onPressedMoveEnd', figure) && typeof overlay.getHasMoved === 'function' && overlay.getHasMoved() === true) {
      ...
      if (overlay !== null) {
        if (overlay.setHasMoved) overlay.setHasMoved(true)
  ...
  private _figureMouseDownEvent (overlay: OverlayImp, figureType: EventOverlayInfoFigureType, figureIndex: number, figure: OverlayFigure): MouseTouchEventCallback {
    return (event: MouseTouchEvent) => {
      ...
      if (overlay.setHasMoved) overlay.setHasMoved(false)
  ...
  private _coordinateToPoint (o: Overlay, coordinate: Coordinate): Partial<Point> {
    ...
    if (this.coordinateToPointValueFlag()) {
      ...
      const value = yAxis.convertFromPixel(coordinate.y)
      point.value = value
    }
  ...
  override dispatchEvent (name: EventName, event: MouseTouchEvent): boolean {
    ...
    const isOverlayDrawing = chartStore.isOverlayDrawing()
    const progressOverlayInfo = chartStore.getProgressOverlayInfo()
    
    // 只在点击事件时检查按钮区域
    if (name === 'mouseClickEvent') {
      // 使用缓存检查按钮区域
      const inButtonArea = this._isMouseInButtonArea(event)
      if (inButtonArea) {
        return false
      }
    }
