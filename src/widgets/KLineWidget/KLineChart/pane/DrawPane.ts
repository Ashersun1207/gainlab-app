...
export default abstract class DrawPane<C extends Axis = Axis> extends Pane {
  ...
  private readonly _options: DeepRequired<PaneOptions> = {
    ...
    state: 'normal',
    axis: { name: 'normal', scrollZoomEnabled: true } as any
  }
  ...
  setOptions (options: PaneOptions): this {
    ...
    // 展开时排除函数类型的 DeepRequired，因为它们不能直接赋值
    const axisOptions: any = { ...this._options.axis }
    // 如果 minSpan 是函数，直接使用；如果是 DeepRequired 包装的，需要提取
    if (axisOptions.minSpan && typeof axisOptions.minSpan === 'function') {
      axisOptions.minSpan = axisOptions.minSpan as any
    }
    if (axisOptions.createRange && typeof axisOptions.createRange === 'function') {
      axisOptions.createRange = axisOptions.createRange as any
    }
    this._axis.override({
      ...axisOptions,
