...
export default abstract class FigureImp<A = unknown, S = unknown> extends Eventful implements Omit<Figure<A, S>, 'name' | 'draw' | 'checkEventOn'> {
  ...
  // 添加getBounding方法，返回一个默认的边界框
  getBounding(): { left: number; top: number; right: number; bottom: number } {
    // 从attrs中获取位置信息，如果没有则返回默认值
  ...
  static extend<A, S> (figure: FigureTemplate<A, S>): new (figure: FigureCreate<A, S>) => FigureImp<A, S> {
