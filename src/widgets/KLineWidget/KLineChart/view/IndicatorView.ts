  ...
  override drawImp (ctx: CanvasRenderingContext2D): void {
    ...
    indicators.forEach(indicator => {
      if (indicator.visible) {
        ...
        // 新增：渲染全局型 figure（如 polygon）
        const result = indicator.result
        const barSpace = chartStore.getBarSpace()
        indicator.figures.forEach((figure, figureIndex) => {
          if (figure.type === 'polygon' && typeof figure.attrs === 'function') {
            // 为全局 polygon figure 提供必要的参数
            // 使用第一个和最后一个数据点作为参考
            const dataLength = result?.length ?? 0
            const firstData = dataLength > 0 ? result[0] : null
            const lastData = dataLength > 0 ? result[dataLength - 1] : null
            const firstCoordinate = dataLength > 0 ? { x: xAxis.convertToPixel(0) } : { x: 0 }
            const lastCoordinate = dataLength > 0 ? { x: xAxis.convertToPixel(dataLength - 1) } : { x: 0 }
            
            const attrs = figure.attrs({
              data: { prev: null, current: firstData, next: lastData },
              coordinate: { prev: null, current: firstCoordinate, next: lastCoordinate },
              bounding,
              barSpace,
              xAxis,
              yAxis
            } as any)
            if (attrs && (attrs as any).coordinates && Array.isArray((attrs as any).coordinates) && (attrs as any).coordinates.length > 0) {
              this.createFigure({
                name: 'polygon',
                attrs,
                styles: typeof figure.styles === 'function'
                  ? figure.styles({ 
                      data: { prev: null, current: firstData, next: lastData },
                      indicator, 
                      defaultStyles 
