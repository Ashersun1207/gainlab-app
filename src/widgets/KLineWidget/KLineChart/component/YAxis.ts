...
export default abstract class YAxisImp extends AxisImp implements YAxis {
  ...
  protected override createRangeImp (): AxisRange {
    ...
    // 处理脚本的minValue和maxValue
    const scripts = chartStore.getScriptsByPaneId(paneId)
    scripts.forEach(script => {
      // 优先使用用户设置的_default值
      if (script._default?.min !== undefined && script._default?.min !== null) {
        specifyMin = Math.min(specifyMin, script._default.min);
      } else if (isNumber(script.minValue)) {
        // 如果没有用户设置，使用自动计算的值
        specifyMin = Math.min(specifyMin, script.minValue);
      }
      
      if (script._default?.max !== undefined && script._default?.max !== null) {
        specifyMax = Math.max(specifyMax, script._default.max);
      } else if (isNumber(script.maxValue)) {
        // 如果没有用户设置，使用自动计算的值
        specifyMax = Math.max(specifyMax, script.maxValue);
      }
      // 处理脚本的精度
      if (script.precision !== null && script.precision !== undefined) {
        if (typeof script.precision === 'number') {
          indicatorPrecision = Math.min(indicatorPrecision, script.precision);
        } else if (script.precision === 'price') {
          const pricePrecision = chartStore.getSymbol()?.pricePrecision ?? 2;
          indicatorPrecision = Math.min(indicatorPrecision, pricePrecision);
        } else if (script.precision === 'volume') {
          const volumePrecision = chartStore.getSymbol()?.volumePrecision ?? 2;
          indicatorPrecision = Math.min(indicatorPrecision, volumePrecision);
        }
      }
    })
    ...
    // 如果有脚本设置了Y轴范围，直接使用脚本的范围
    if (specifyMin !== Number.MAX_SAFE_INTEGER && specifyMax !== Number.MIN_SAFE_INTEGER) {
      min = specifyMin;
      max = specifyMax;
    } else {
      // 否则使用原有的数据计算逻辑
    ...
    const isArea = candleStyles.type === 'area' || candleStyles.type === 'price_line'
  ...
  protected override createTicksImp (): AxisTick[] {
    ...
    if (this.isInCandle()) {
      ...
      scripts.forEach(script => {
        if (script.precision !== null && script.precision !== undefined) {
          if (typeof script.precision === 'number') {
            precision = Math.max(precision, script.precision);
          } else if (script.precision === 'price') {
            const pricePrecision = chartStore.getSymbol()?.pricePrecision ?? 2;
            precision = Math.max(precision, pricePrecision);
          } else if (script.precision === 'volume') {
            const volumePrecision = chartStore.getSymbol()?.volumePrecision ?? 2;
            precision = Math.max(precision, volumePrecision);
          }
        }
      })
    }
