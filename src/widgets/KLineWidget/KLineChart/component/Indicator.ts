...
/**
 * Indicator input parameter type
 */
...
export type IndicatorRegenerateFiguresCallback<D, C> = (inputs: C[]) => Array<IndicatorFigure<D>>
...
export type IndicatorCalcCallback<D, C, E> = (
  dataList: KLineData[],
  indicator: Indicator<D, C, E>,
  prevResult?: D[],
  type?: 'init' | 'append' | 'tick'
) => Promise<D[]> | D[]
...
export default class IndicatorImp<D = unknown, C = unknown, E = unknown> implements Indicator<D, C, E> {
  ...
  shouldUpdate: IndicatorShouldUpdateCallback<D, C, E> = (prev, current) => {
    const calc = JSON.stringify(prev.inputs) !== JSON.stringify(current.inputs) ||
  ...
  override (indicator: Partial<Indicator<D, C, E>>): void {
    ...
    if (isValid(inputs)) {
      ...
      if (isFunction(this.regenerateFigures)) {
        this.figures = this.regenerateFigures(this.inputs)
      }
    }
  ...
  async calcImp (dataList: KLineData[], prevResult?: D[], type: 'init' | 'append' | 'tick' = 'init'): Promise<boolean> {
    try {
